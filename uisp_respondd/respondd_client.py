...
class ResponddClient:
    ...
    def getNeighbours(self):
        ...
        for ap in aps.accesspoints:
            nbs = {}
            if ap.neighbour is not None:
                for neighbour in aps.accesspoints:
                    if neighbour.name == ap.neighbour:
                        logger.info(neighbour.mac)
                        nbs[neighbour.mac] = NeighbourDetails(tq=255, lastseen=0.45)
                        neighbours.append(
                            NeighboursInfo(
                                node_id=ap.mac.replace(":", ""),
            ...
            statistics.append(
                StatisticsInfo(
                    ...
                    traffic=(
                        TrafficInfo(
                            tx=txInfo(
                                bytes=ap.tx_bytes if ap.tx_bytes is not None else 0
                            ),
                            rx=rxInfo(
                                bytes=ap.rx_bytes if ap.rx_bytes is not None else 0
                            ),
                        )
                        if has_traffic
                        else None
    ...
    def start(self):
        ...
        while True:
            ...
            if self._config.multicast_enabled:
                ...
                self.sendUnicast()
            self._timeStart = time.time()
            self._aps = uisp_client.get_infos()
